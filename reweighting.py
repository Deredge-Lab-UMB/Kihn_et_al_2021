###
Script used to execute the HDXer post hoc reweighing described in Bradshaw et. al., 2020 
(liveComs). Script was implemented in the reweighing of the PluS WT and H212R simulation in Kihn et al., 2021.
###
...
def parse():
    ...
    parser.add_argument("-exp","--expfile", \
                        help="Path of file with experimental deuterated fractions. Fitting will be performed to the segments in this file. Segs should be defined one per line, followed by one column for each timepoint in --times, Whitespace separated. No default.", \
                        required=True)
    parser.add_argument("-norw","--no_reweight", \
                        help="Switch OFF reweighting of trajectory frames to fit experiment. By default reweighting is ON", \
                        default=True, dest='do_reweight', action='store_false')
    parser.add_argument("-nopm","--no_params", \
                        help="Switch OFF optimization of beta parameters to fit experiment. By default optimization is ON", \
                        default=False, dest='do_params', action='store_false')
    parser.add_argument("-mcmin","--mc_minimization", \
                        help="Switch ON Montecarlo optimization of beta parameters to fit experiment. By default MC optimization is OFF", \
                        default=False, dest='do_mc', action='store_true')
    parser.add_argument("-mcsampl","--mc_sampling", \
                        help="Switch ON Montecarlo sampling of beta parameters (error variance can be provided using -refvar). By default MC sampling is OFF", \
                        default=False, dest='do_mcsmpl', action='store_true')
    parser.add_argument("-refvar", "--refvar", help="Reference variance of MC sampling distribution (estimated error on deuterated fractions) . Default=0.03", \
                        type=float, default=0.03)
    parser.add_argument("-equilsteps", "--equilsteps", help="Equilibration steps and subsequent smoothing (as 1/sqrt(number of steps)) for MC sampling. Default=none (-1)", \
                        type=float, default=-1)
#    parser.add_argument("-r","--ratefile", \
#                        help="Path of file with intrinsic rates. These should be defined as '$resid $rate', whitespace separated, with one line pre residue. If the input trajectories do not have the same number of residues, add one file per topology here. The trajectories will be fitted based on identified common residues only. No default.", \
#                        nargs='+', required=True)
    parser.add_argument("-r","--ratefile", \
    ...
    parser.add_argument("-out", "--outprefix", \
                        help="Prefix for output files. Default none.", \
                        type=str, default='')
    parser.add_argument("-bc", "--bc", help="Initial beta-C parameter for calculating ln(Pf). Default=0.35", \
                        type=float, default=0.35)
    parser.add_argument("-bh", "--bh", help="Initial beta-H parameter for calculating ln(Pf). Default=2.00", \
                        type=float, default=2.00)
    parser.add_argument("-bcrange", "--bcrange", help="beta-C parameter variation range for MC minimization. Default=1.5", \
                        type=float, default=1.5)
    parser.add_argument("-bhrange", "--bhrange", help="beta-H parameter variation range for MC minimization. Default=16.00", \
                        type=float, default=16.00)
    parser.add_argument("-g", "--gamma", help="Gamma parameter for reweighting. Default=1.00", \
                        type=float, default=1.00)
    parser.add_argument("-tol", "--tolerance", help="Convergence tolerance for lambdas and betas. Default=10**-10", \
                        type=float, default=10**-10)
    parser.add_argument("-it", "--maxiters", help="Maximum iterations for reweighting. Default=10^6", \
                        type=int, default=10**6)
    parser.add_argument("-pmit", "--parammaxiters", help="Maximum iterations for parameter optimization. Default=10^2", \
                        type=int, default=10**2)
    parser.add_argument("-rf", "--ratef", help="Initial rate (step size) for reweighting. Default=10^-5", \
                        type=float, default=10**-5)
    parser.add_argument("-sf", "--scaleff", help="Ratio for scaling down of initial step size if oscillations are observed. Default=1.005", \
                        type=float, default=1.005)
    parser.add_argument("-pf", "--paramf", help="Maximum rate step size for parameter optimization. Default=10^-1", \
                        type=float, default=10**-1)
    parser.add_argument("-tm", "--temp", help="Temperature. Default=300 K", \
                        type=float, default=300.)
    parser.add_argument("-ran", "--randomweights", help="Randomize the initial weights of the trajectory frames (alternately they will be treated as equal). Default=False", \
                        dest='randomweights', action='store_true', default=False)
    parser.add_argument("-rst", "--restart", help="Name of the pkl restart file with parameters for continuing reweighting. Note that only the following command line arguments will actually override what's present in the pkl: ", \
                        type=str)
...
#        contactfiles.append(sorted(glob(os.path.join(folder, "Contacts_chain_1*.tmp")), key=lambda x: strip_filename(x, extrastr="chain_1_res_")))
        hbondfiles.append(sorted(glob(os.path.join(folder, "Hbonds_chain_0*.tmp")), key=lambda x: strip_filename(x, extrastr="chain_0_res_")))
#        hbondfiles.append(sorted(glob(os.path.join(folder, "Hbonds_chain_1*.tmp")), key=lambda x: strip_filename(x, extrastr="chain_1_res_")))
...
def setup_no_restart():
    ...
    # if optionally average over parameters, define _lambdanewaver

#    if mc_sample_params:
    global _lambdanewaver, _lambdanew, chisquareav
    _lambdanewaver=np.zeros(nresidues)
...
def setup_restart(rstfile):
    ...
    global contacts, hbonds, kint, exp_dfrac, segfilters, lambdas, _lambdanewaver, _lambdanew, chisquareav, \
           nframes, iniweights, num, exp_df_segf, normseg, numseg, lambdamod, deltalambdamod, \
           Bc, Bh, gamma, ratef, avesigmalnpi, currcount
    contacts, hbonds, kint, exp_dfrac, segfilters, lambdas, _lambdanewaver, _lambdanew, chisquareav, \
    nframes, iniweights, num, exp_df_segf, normseg, numseg, lambdamod, deltalambdamod, \
...
for ncount in range(currcount+1,count+1):
    ...
    if mc_sample_params:
       ...
       for iterpar in range(0,paramcount):
           ...
           like=numseg*chisquarenew/(2.0*chisquareref)
           likeold=numseg*chisquare/(2.0*chisquareref)
           if Bhnew>0:
              if Bcnew>0:
           ...
           if update_lambdas:
              ...
              _lambdanewaver=_lambdanewaver+_lambdanew        
       Bh=Bhold*(1.0-parrate)+parrate*(Bhaver/paramcount)
       Bc=Bcold*(1.0-parrate)+parrate*(Bcaver/paramcount)
       _lambdanew=_lambdanewold*(1.0-parrate)+parrate*(_lambdanewaver/paramcount)
...
# Update avehdxdev & chisquare with final segfracs
    ...
    if (ncount % 100) == 0:
        ...
        paramlist = [ contacts, hbonds, kint, exp_dfrac, segfilters, lambdas, _lambdanewaver, _lambdanew, chisquareav, \
                      nframes, iniweights, num, exp_df_segf, normseg, numseg, lambdamod, deltalambdamod, \
