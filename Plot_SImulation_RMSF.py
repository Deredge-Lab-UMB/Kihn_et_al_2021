...
###
Plots per residue average and maximum RMSF of trajectories from the simulation starting structure.
###

import pandas as pd
import seaborn as sns
import matplotlib.pyplot as plt
df = pd.read_csv('RMSF_H212R_Run1.tml', skiprows=[0,1,2,3,4,5,6,7,8], header=None, sep='\s+')
...
plt.xlabel('Frame')
plt.ylabel('Residue')
plt.savefig('Heatmap_RMSF.png', dpi=600)
plt.close()
ax = []
mx = []
y_ax = []
...
plt.savefig('Average_RMSF_Plot.png', dpi=600)
plt.close()
plt.plot(mx, y_ax)
plt.gca().invert_yaxis()
plt.savefig('Max_RMSF_Plot.png', dpi=600)
plt.close()
with open('RMSF_Results.txt', 'w+') as f:
    f.write('PhuS  H212R 520K Simulated Tempering Run 1 RMSF Avereage: ' + str(sum(ax)/len(ax)) + '\n')
    f.close()
